...
class GALoraConfig(LoraConfig):
    group_size: int = field(default=32, metadata={"help": "GALora group size"})
    mamba_depth: int = field(default=1, metadata={"help": "Mamba depth"})
...
class GALoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, group_size=32, mamba_depth=0):
        ...
        # == Mamba Start ==
        if mamba_depth >= 1:
            self.mamba_group[adapter_name] = VSSLayer(L, mamba_depth)
        # == Mamba End ==
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in self.lora_A.keys():
                    continue
                # == GA Start ==
                ga_group = getattr(self.ga_group, active_adapter, None)
                # == GA End ==
                # == Mamba Start ==
                mamba = getattr(self.mamba_group, active_adapter, None)
                # == Mamba End ==
