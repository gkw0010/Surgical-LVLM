...
class EndoVis18VQAClassification(Dataset):
    ...
    def __init__(self, seq, folder_head, folder_tail, patch_size=4, robustpath=None):
        ...
        # print(folder_head + str(curr_seq) + folder_tail)
        self.vqas = []
    ...
    def __getitem__(self, idx):
        ...
        visual_feature_loc = os.path.join(loc[0],loc[1],loc[2],loc[3], 'img_features', (str(self.patch_size)+'x'+str(self.patch_size)),loc[-1].split('_')[0]+'.hdf5')   
        # visual_feature_loc = os.path.join(loc[0],loc[1],loc[2], 'vqa/img_features/frcnn_fixed',loc[-1].split('_')[0]+'.hdf5')
        frame_data = h5py.File(visual_feature_loc, 'r')    
        visual_features = torch.from_numpy(frame_data['visual_features'][:])
            
        # question and answer
        question = self.vqas[idx][1].split('|')[1]
...
class EndoVis17VQAClassificationValidation(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img
        loc = self.vqas[idx][0].split('/')
        visual_feature_loc =  os.path.join(loc[0],loc[1],loc[2], 'vqla/img_features', (str(self.patch_size)+'x'+str(self.patch_size)),loc[-1].split('.')[0]+'.hdf5')
        # visual_feature_loc = os.path.join(loc[0],loc[1], 'vqla/img_features/frcnn_fixed', loc[-1].split('.')[0]+'.hdf5')
...
class QWEN18ClassificationValidation(Dataset):
    ...
    def __init__(self, external_folder_head, external_folder_tail, patch_size=4, robustpath=None):
        ...
        for line in lines: 
             self.vqas.append([line.split("', '")[0].replace("['", ''), line.split("', '")[1].replace("']\n", '')])
    ...
    def __getitem__(self, idx):
        ...
        # img
        loc = self.vqas[idx][0].split('/')
        visual_feature_loc =  os.path.join(loc[0],loc[1],loc[2], 'vqa/img_features', (str(self.patch_size)+'x'+str(self.patch_size)),loc[-1].split('.')[0]+'.hdf5')
        # visual_feature_loc = os.path.join(loc[0],loc[1], 'vqla/img_features/frcnn_fixed', loc[-1].split('.')[0]+'.hdf5')
        ...
        label = self.labels.index(str(self.vqas[idx][1].split('|')[0]))
        
        # detection bbox
        bbox_label = self.vqas[idx][1].split('|')[1].replace(' ','').replace('\n', '').split(',')
        bbox_label_xyxy = torch.tensor(list(map(int, bbox_label))) - torch.tensor([1, 1, 1, 1])
        bbox_label_cxcywh = box_xyxy_to_cxcywh(bbox_label_xyxy)
        bbox_label_cxcywh_nomalize = bbox_label_cxcywh / torch.tensor([1280, 1024, 1280, 1024])
        return loc[-1].split('_')[0], visual_features, question, label, bbox_label_cxcywh_nomalize
...
class QWEN17ClassificationValidation(Dataset):
