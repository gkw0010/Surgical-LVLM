...
from models.LViTPrediction_MUTAN import LViTPrediction_MUTAN
from models.LViTPrediction_BlockTucker import LViTPrediction_BT
from models.LViTPrediction_MFH import LViTPrediction_MFH
from models.LViTPrediction import LViTPrediction
from models.DeiTPrediction_DeiT import DeiTPrediction_DeiT
import warnings
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'endovis':
        ...
        '''
        # tokenizer
        tokenizer = BertTokenizer.from_pretrained("bertvocab_extended_infu/v2/")
        ...
        folder_head = 'datasets/EndoVis-18-VQA_extended/seq_'
        ...
        # external dataloader
        external_folder_head = 'datasets/EndoVis-17-VQA_extended_final/'
        ...
        qwen_val_dataset = QWEN17ClassificationValidation(external_folder_head, external_folder_tail, patch_size = args.patch_size, robustpath=args.robustpath)
        qwen_val_dataloader = DataLoader(dataset=qwen_val_dataset, batch_size= args.batch_size, shuffle=False)
    ...
    if args.checkpoint is None:
        ...
        if args.model == 'vb':
        ...
        elif args.model == 'vbrm':
            model = VisualBertResMLPPrediction(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class, token_size = int(args.question_len+ (args.patch_size * args.patch_size)))  # (args.patch_size * args.patch_size))) # args.question_len+ 36
        elif args.model == 'cat':
            ...
            model = LViTPrediction_MUTAN(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class)
        elif args.model == 'bt':
            model = LViTPrediction_BT(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class)
        elif args.model == 'mfh':
            model = LViTPrediction_MFH(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class)
        elif args.model == 'gvle':
            ...
            model = DeiTPrediction_DeiT(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class)
    ...
    # validation
    if args.validate:
        checkpoint_path = args.checkpoint_dir + '/_Epoch14_Best.pth'
        ...
        test_acc, test_c_acc, test_precision, test_recall, test_fscore, test_bbox_miou = validate(args, val_loader=qwen_val_dataloader, model = model, criterion=criterion, epoch=(args.epochs-1), tokenizer = tokenizer, device = device)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            checkpoint_path = args.checkpoint_dir + '_Epoch'+ str(epoch) + '_Best.pth'
